...
```yaml
检测逻辑:
  1. 读取 examples/README.md 中声明的示例
  2. 验证每个示例目录是否存在且完整
...
function detectCompleteness() {
  ...
  for (const [workflow, templates] of Object.entries(workflowTemplateMap)) {
    const workflowPath = `workflows/${workflow}`;
    ...
    for (const template of templates) {
      const templatePath = `templates/core/${template}`;
      const extendedPath = `templates/extended/${template}`;
...
| 文件类型 | 必要章节 | 可选章节 |
|---------|---------|---------|
| standards/*.md | 标题、规则、示例、检查清单 | 最佳实践、FAQ |
| workflows/*.md | 流程概览、步骤、输出 | 异常处理、示例 |
| templates/*.md | 头部、章节结构、占位符 | 填写指南 |

### 目录完整性标准

| 目录 | 必须包含 | 可选 |
|-----|---------|------|
| workflows/ | README.md, 00-08 子目录 | common/ |
| templates/ | README.md, core/, lite/ | extended/ |
| projects/ | README.md, _template/ | 示例项目 |
| standards/ | README.md, 9 个规范文件 | - |
