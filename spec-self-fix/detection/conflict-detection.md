...
检测逻辑:
  ...
  6. 预检查第 6 行（📝 记忆已创建）在 3 处文件中是否都存在且格式一致:
     a. workflows/00-pre-check/README.md §检查清单 + §输出格式（标准/扩展）
     b. QUICK-REFERENCE.md §预检查模板
     c. workflows/00-pre-check/memory-and-rules.md §阶段 0 输出
  7. 第 6 行格式是否统一为:
...
检测逻辑:
  1. 确认以下 3 处文件都包含混合意图优先级规则:
     a. workflows/00-task-identification/rules.md §混合意图优先级规则
     b. workflows/01-requirement-dev/README.md §强制执行规则（混合意图判定）
     c. workflows/10-analysis/README.md §变更意图检测
...
v2.2 (2026-02-28):
  ...
  根因:
    ...
    - 需要检测规则确保 4 处定义一致存在
  关联:
    - workflows/00-task-identification/rules.md §混合意图优先级规则
    - workflows/01-requirement-dev/README.md §强制执行规则
    - workflows/10-analysis/README.md §变更意图检测
