...
关联:
  ...
  - workflows/common/task-memory.md §阶段 0 时序强制规则（权威来源）
  - workflows/00-pre-check/README.md §阶段 0 时序强制规则（同步副本）
```
...
同步目标:
  ...
  自动同步更新以下引用处:
  1. spec-self-fix/detection/conflict-detection.md §规则 1 的 8 文件清单
  2. spec-self-fix/repair/repair-patterns.md §模式 5 的 8 文件清单
  3. spec-self-fix/repair/auto-repair.md §场景 A 的 8 文件清单
  4. spec-self-fix/triggers/auto-triggers.md §场景 5 的 8 文件清单
