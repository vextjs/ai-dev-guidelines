...
限制操作:
  - 不能直接 commit（使用 report_progress）
  - 不能 push（使用 report_progress）
  - 不能修改 commit 历史（rebase/reset）
  - 不能切换分支

原因:
  AI 没有 GitHub 凭证，必须通过 report_progress 工具提交
```
