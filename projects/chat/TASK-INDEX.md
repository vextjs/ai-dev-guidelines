# 任务索引

> **用途**: 快速追溯项目的所有历史任务  
> **更新时机**: 每次任务完成后 AI 自动更新  
> **最后更新**: 2026-02-28

---

## ⚠️ 强关联文件

本文件与以下文件**强关联**，修改时需同步考虑：

| 关联文件 | 关联关系 |
|---------|---------|
| `core/workflows/01-requirement-dev/README.md` | 需求完成后更新本索引 |
| `core/workflows/02-bug-fix/README.md` | Bug修复后更新本索引 |
| `core/workflows/03-optimization/README.md` | 优化完成后更新本索引 |
| `core/workflows/04-research/README.md` | 调研完成后更新本索引 |
| `core/workflows/05-refactoring/README.md` | 重构完成后更新本索引 |
| `core/workflows/06-database/README.md` | 数据库变更后更新本索引 |
| `core/workflows/07-security/README.md` | 安全修复后更新本索引 |
| `core/workflows/08-incident/README.md` | 事故复盘后更新本索引 |
| `QUICK-REFERENCE.md` | 定义追溯查询方式 |

---

## 📋 需求开发

| ID | 日期 | 标题 | 状态 | 路径 |
|----|------|------|------|------|
| REQ-001 | YYYY-MM-DD | [示例] 功能名称 | ✅ 完成 | `requirements/YYYYMMDD-feature-name/` |

---

## 🐛 Bug 修复

| ID | 日期 | 标题 | 严重程度 | 状态 | 路径 |
|----|------|------|---------|------|------|
| BUG-001 | YYYY-MM-DD | [示例] Bug 描述 | P1 | ✅ 修复 | `bugs/BUG-001-xxx/` |

---

## ⚡ 性能优化

| ID | 日期 | 标题 | 提升效果 | 状态 | 路径 |
|----|------|------|---------|------|------|
| OPT-001 | 2026-02-28 | 创建项目接口 AI 标题生成改异步 | 响应时间 2~10s → <500ms | ✅ 已执行（待验证） | `optimizations/创建项目AI标题异步化/README.md` |

---

## 🔬 技术调研

| ID | 日期 | 标题 | 结论 | 路径 |
|----|------|------|------|------|
| RES-001 | YYYY-MM-DD | [示例] 调研主题 | 选择方案A | `research/RES-001-xxx/` |

---

## 🏗️ 架构重构

| ID | 日期 | 标题 | 状态 | 路径 |
|----|------|------|------|------|
| REF-001 | YYYY-MM-DD | [示例] 重构描述 | ✅ 完成 | `refactoring/REF-001-xxx/` |

---

## 🚨 事故复盘

| ID | 日期 | 标题 | 级别 | 路径 |
|----|------|------|------|------|
| INC-001 | YYYY-MM-DD | [示例] 事故描述 | P0 | `incidents/INC-001-xxx/` |

---

## 🔍 快速检索

### 按关键词查找

如需查找特定功能的历史记录，可以：
1. 在本文件中搜索关键词
2. 找到对应的路径
3. 读取详细文档

### 按时间查找

按日期倒序排列，最新的任务在最上面。

---

**维护说明**: 此文件由 AI 在任务完成时自动更新，请勿手动修改。

