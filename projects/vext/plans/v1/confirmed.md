...
> **项目**: vext
> **日期**: 2026-02-26（最后更新: 2026-03-03 S1-S7 改进建议 Q46-Q52）
> **状态**: 🔄 部分确认，持续更新中
...
| Q44 | P1: `docs.responses` 响应配置 | 支持按 HTTP 状态码定义响应（`description` + `schema` + `headers` + `example` + `examples`）；schema 格式与 validate 一致（schema-dsl DSL 字符串对象）或 JSON Schema 对象或 `$ref` 引用字符串；成功响应（2xx）自动包装为 vext 标准格式 `{ code: 0, data: <schema>, requestId }`，错误响应（4xx/5xx）直接使用原始 schema；204 No Content 无响应体；未声明 responses 时默认添加 200 OK 引用 `SuccessResponse` schema。变更文件：`14-openapi.md` §1 | ✅ 已确认（`14-openapi.md` §1）|
| Q45 | P0-3: `VextMiddleware` next 类型修复 | `next` 类型从 `() => void` 改为 `() => Promise<void>`，与 `executeChain` 内部 `const next = async ()` 的实际实现对齐。用户中间件应 `await next()` 以支持洋葱模型 after-middleware 逻辑（耗时统计/after-response logging 等）。修复了类型签名与运行时实现的不一致，避免 after-middleware 竞态。变更文件：`03-middlewares.md` §4（类型定义+所有示例）、`01b-middlewares.md` §4.1/§4.2/§4.4（所有示例）、`08-adapter.md` §3 executeChain + 附录类型索引、`04-plugins.md` §3.1、`06a-logger.md` §4、`06b-error.md` §1.7/setRateLimiter、`06c-lifecycle.md` §3.2、`01a-validate.md` §4（内部实现+文字描述）。 | ✅ 已确认（2026-03-03 webstorm-copilot）|
| Q46 | S1: schema-dsl 防腐层 | Phase 1 新增任务 1.0：`lib/schema-adapter.ts`，封装 `dsl()`/`I18nError.create()`/`dsl.config()` 调用。validate 中间件（1.13）、OpenAPI SchemaConverter（2.6）、i18n-loader（1.17）、default-throw（1.8）统一通过此模块访问 schema-dsl，隔离 breaking change 风险。拓扑图 Level 1 新增 `schema-adapter`。估时 0.5d。变更文件：`IMPLEMENTATION-PLAN.md`（新增任务 1.0 + 拓扑图 Level 1 + 任务 1.8/1.13/1.17/2.6 说明补充防腐层引用） | ✅ 已确认（2026-03-03 zed-copilot）|
| Q47 | S2: Phase 2 拆分为 2A + 2B | 原 Phase 2（8 任务，4 周）拆分为：**Phase 2A**（v0.2.0-alpha，5 任务，3 周）= Cold Restart + vext build + OpenAPI；**Phase 2B**（v0.2.0，4 任务，3 周）= Soft Reload Tier 1/2 + access-log + CLI 升级集成。新增任务 2.8（升级 CLI 集成 Soft Reload，1.5d）。降级策略：Phase 2B 受阻时可推迟至 v0.3.0，Phase 2A 的 Cold Restart 已提供基本可用的开发体验。变更文件：`IMPLEMENTATION-PLAN.md`（Phase 2 重组 + 新增降级策略 + 完成标准拆分） | ✅ 已确认（2026-03-03 zed-copilot）|
| Q48 | S3: Phase 1 增加 1 周缓冲 | Phase 1 工期从 6 周延长为 7 周（含 1 周缓冲 W9）。关键路径约 8.5 周，24 个串行任务有累积延期风险，缓冲周用于吸收延期。如无延期，可提前进入 Phase 2A 或完善测试覆盖率。变更文件：`IMPLEMENTATION-PLAN.md`（Phase 1 标题 + 新增 W9 缓冲周说明） | ✅ 已确认（2026-03-03 zed-copilot）|
| Q49 | S4: 文档站提供第三方集成示例 | Phase 4 任务 4.3 文档站新增内容：Zod 校验器集成示例（通过 `app.setValidator()` 切换）、Drizzle ORM 插件示例、Prisma 插件示例。降低自研组件（schema-dsl/monSQLize）的生态风险，让用户知道可以自由选择主流替代品。5d 产出基础版，后续迭代补充。变更文件：`IMPLEMENTATION-PLAN.md`（任务 4.3 说明 + Phase 4 完成标准 + 风险矩阵） | ✅ 已确认（2026-03-03 zed-copilot）|
| Q50 | S5: app.fetch 拓扑位置调整 | `app.fetch`（任务 1.8b）从拓扑 Level 1 移至 Level 3，从 W3-W4 移至 W5-W6。原因：app.fetch 的 requestId 自动传播功能从 `requestContext.getStore()` 读取 requestId，但 Level 1 阶段 requestContext 虽已创建（AsyncLocalStorage 实例），store 尚未被 requestId 中间件（Level 3）运行时填充。将 app.fetch 与 requestId 中间件同层（Level 3）可确保测试和开发时依赖关系清晰。变更文件：`IMPLEMENTATION-PLAN.md`（任务 1.8b 移动 + 拓扑图 Level 1→Level 3 + 拓扑变更说明表） | ✅ 已确认（2026-03-03 zed-copilot）|
| Q51 | S6: SSE 推迟到 v1.1.0 | 原 Phase 3 任务 3.6（SSE 支持 `res.sse()` + 连接管理，2d）移至 v1.1.0 路线图。原因：SSE 非 MVP 必需，`res.stream()` 已预留接口基础，不阻塞 v1.0.0 发布。Phase 3 任务数 6→5，工期 4 周→3 周。新增 v1.1.0 路线图章节（含 SSE/AOP/WebSocket/多语言文档）。变更文件：`IMPLEMENTATION-PLAN.md`（Phase 3 移除 3.6 + 新增 v1.1.0 路线图 + Phase 3 完成标准移除 SSE） | ✅ 已确认（2026-03-03 zed-copilot）|
| Q52 | S7: service-loader 运行时循环依赖检测 | 任务 1.12 service-loader 新增运行时循环依赖检测能力。加载完成后通过静态分析 service 构造函数中 `this.app.services.xxx` 的调用模式构建依赖图，检测环路并 Fail Fast。补强架构约束第 7 条（禁止 service 循环依赖）的运行时保障——原先仅靠文档约束，无运行时检测。估时从 1d 调整为 1.5d。变更文件：`IMPLEMENTATION-PLAN.md`（任务 1.12 说明 + 拓扑图标注 + Phase 1 完成标准）、`02-services.md`（§7 Fail Fast 验证表新增循环依赖检测项） | ✅ 已确认（2026-03-03 zed-copilot）|
