...
> 🆕 **v2.5 更新**：Phase 4.3 文档站 ✅ 完成（39 页 Rspress SSG + GitHub Pages）；v0.1.0→v0.1.1→v0.1.2 已发布；BUG-004/005 已修复（config-loader middlewares 丢失 + app.fetch 挂载时序）
...
Phase 4:  正式版 v1.0.0     [ 6/7]  ████████░░  86%   🔄 进行中
─────────────────────────────────────────────
总计:                       [51/52] █████████░  98%
v1.1.0 路线图:              [ 2/8]  ██░░░░░░░░  25%   🔄 部分完成
```

> **已发布版本**: v0.1.0 (2026-03-04) → v0.1.1 (2026-03-05) → v0.1.2 (2026-03-05)
> **已修复发版后 Bug**: BUG-001(CJS/ESM) + BUG-002(文档 decorate→extend) + BUG-003(emoji 乱码) + BUG-004(middlewares 白名单丢失) + BUG-005(app.fetch 挂载时序)
...
| 4.3 | 文档站（rspress）— Guide + API + 示例 + 🆕 第三方集成示例 | ✅ | 🔴 | 5d | 2026-03-05 完成（会话 03→05→06→07→10→11→12）。**工具选型**: Rspress v2（基于 Rsbuild 的高性能 SSG）。**独立目录** `website/`（项目仓库内，有自己的 `package.json` 不污染主包）。**最终站点结构（39 页）**: `docs/index.md`（首页 Hero + Features）+ `docs/guide/`（**22 篇指南**：introduction / quick-start / project-structure / routing / services / middleware / plugins / validation / configuration / adapters / openapi / testing / cli / hot-reload / cluster / i18n / fetch / database / build / deployment / logger / request-context）+ `docs/api/`（**7 篇 API 参考**：config / route-definition / context / app / plugin-api / testing-api / fetch）+ `docs/examples/`（**7 篇示例**：hello-world / crud-api / zod-validation / drizzle-orm / prisma-orm / nacos-integration / opentelemetry）+ `docs/benchmark.md`（性能数据）。**额外完成项**: 文档完整性审查（28 篇 vs 22 模块，覆盖率 95.5%，综合 4.6/5.0）+ GitHub Pages 部署 workflow（`.github/workflows/deploy-docs.yml`）+ 自定义 SVG favicon + CI docs-build 门禁（PR check）+ 搜索功能优化。**构建验证**: rspress build 通过零错误。Git 多次提交+推送 |
...
✅ benchmark 报告已输出
✅ vext create 可用
✅ 文档站上线（含 Zod/Drizzle/Prisma 第三方集成示例 + Nacos/OpenTelemetry）
⬜ npm publish vextjs@1.0.0 成功（当前已发布 v0.1.2 预览版）
```

> **发版后修复记录**（v0.1.2 发布后发现并修复的 Bug）:
>
> | Bug ID | 严重度 | 描述 | 修复 commit | 状态 |
> |--------|:------:|------|------------|:----:|
> | BUG-001 | P0 | `vext dev` CJS/ESM 不兼容 — dev 编译器输出 CJS 但 vextjs 为 ESM-only | `849984b` | ✅ v0.1.2 |
> | BUG-002 | P1 | 文档 `app.decorate()` → `app.extend()` | `849984b` | ✅ v0.1.2 |
> | BUG-003 | P1 | hot-reload 日志 emoji 乱码（Windows 非 Unicode 终端） | `4678996` | ✅ v0.1.2+ |
> | BUG-004 | P0 | config-loader `deepMerge` 跳过 `middlewares` 键，用户 default.ts 白名单丢失 → 路由级中间件 "not registered" | `db7f20d` | ✅ v0.1.3 |
> | BUG-005 | P1 | `app.fetch` 在 `loadRoutes` 之后才挂载，路由闭包中 `app.fetch` 为 undefined | `db7f20d` | ✅ v0.1.3 |
...
> 🆕 **v2.5 更新**（2026-03-05）：v0.1.0→v0.1.1→v0.1.2 已发布；BUG-004/005 修复；vext-test 44/44 功能验证全通过
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v2.5 | 2026-03-05 | **Phase 4.3 文档站 ✅ 完成 + 发版记录 + Bug 修复**。①Phase 4.3 文档站标记完成（39 页 Rspress SSG：Guide 22 篇 + API 7 篇 + Examples 7 篇 + Benchmark + 首页，含 app.fetch/MonSQLize/deployment/logger/request-context 等增量文档 + 文档完整性审查 28 篇 vs 22 模块覆盖率 95.5%）。②**v0.1.0→v0.1.1→v0.1.2 发版记录**：v0.1.0 初始发布（Phase 0-3）→ v0.1.1（脚手架+安全+社区+文档站）→ v0.1.2（双包 ESM+CJS + 类型增强 + 插件生命周期钩子）。③**BUG-004 P0 修复**：`config-loader.ts` 的 `deepMerge(DEFAULT_CONFIG, userDefaultConfig)` 跳过 `middlewares` 键（设计用于 env/local overlay），导致用户 `config/default.ts` 中的 `middlewares` 白名单丢失 → `loadMiddlewares` 收到空数组 → 路由引用中间件报 "not registered"。修复：deepMerge 后手动合并 `userDefaultConfig.middlewares`。④**BUG-005 P1 修复**：`bootstrap.ts` 中 `app.fetch = createVextFetch(...)` 在 `loadRoutes` 之后才赋值，但 `executeRouteFactory` 将 app 属性拷贝到 collector 代理对象（路由 handler 闭包中的 `app`），此时 `app.fetch` 尚未赋值 → `collector.fetch = undefined` → 路由中调用 `app.fetch()` 报 TypeError。修复：将 `app.fetch` 挂载移到 `loadRoutes` 之前。⑤**验证**：vext 框架 1927/1927 测试零回归 + vext-test 44/44 功能验证 100% 通过（12 大类：路由 6 + 校验 6 + 服务 5 + 中间件 3 + 插件 7 + i18n 4 + fetch 2 + 上下文 2 + OpenAPI 2 + 错误 5 + 配置 1 + 生命周期 1）。commit `db7f20d` pushed。⑥Phase 4 进度 5/7→6/7 (86%)；总进度 50/52→51/52 (98%)；v1.1.0 路线图 1/8→2/8 (25%) |
