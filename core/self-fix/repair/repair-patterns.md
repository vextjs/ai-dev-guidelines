...
示例:
  检测: core/workflows/04-research/README.md 缺少 "## ✅ 完成检查清单"
  模板: core/templates/common/checklist.md
  插入位置: 最后一个 "---" 之前
  结果: 添加标准检查清单章节
```
...
示例:
  ...
  搜索: 查找 "old" 相关文件
  找到: ./core/templates/core/old-template.md
...
```yaml
触发条件: core/standards/README.md 标注可覆盖，但规范文件无标记

安全等级: ✅ 安全

执行步骤:
  1. 读取 core/standards/README.md 的可覆盖列表
  2. 检查每个规范文件是否有标记
  3. 在缺少标记的文件头部添加
...
示例:
  依赖链:
    README.md → core/workflows/common/base.md → core/templates/base-template.md
                                           ↑ 不存在

  方案:
    [A] 创建 core/templates/base-template.md
    [B] 更新 core/workflows/common/base.md 删除此引用
```
...
断层检测清单（6 项）:
  ...
  □ 修复是否回写到 core/self-fix/detection/？（检测规则是否更新）
  □ 修复是否回写到 core/self-fix/triggers/？（自动触发场景是否新增）
  □ 修复是否回写到 core/self-fix/repair/？（修复模式是否更新）
  □ 修复记录是否更新到 core/self-fix/records/？（修复记录和 summary.md）
  □ .ai-memory 记忆文件是否更新？
...
修复目标:
  ...
  - core/workflows/00-pre-check/README.md §检查清单 + §输出格式（标准/扩展）（权威来源）
  - QUICK-REFERENCE.md §预检查模板
  - core/workflows/00-pre-check/memory-and-rules.md §阶段 0 输出
