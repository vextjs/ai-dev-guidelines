...
function main() {
  ...
  for (const [dir, config] of Object.entries(EXPECTED_STRUCTURE)) {
    ...
    if (exists) {
      ...
      issues.all_checks.push({ item: dir, status: config.required ? 'missing_required' : 'missing_optional' });
    }
  }
  console.log('');
  ...
  const yamlFiles = countFiles(ROOT_DIR, '.yaml') + countFiles(ROOT_DIR, '.yml');
