...
| 阶段 | 触发时机 | 做什么 |
|:----:|---------|--------|
| 0 | 收到首条消息 | 预检查 + 创建/追加记忆（← 入口门禁） |
| 1 | 用户发消息 | 捕获用户输入到对话记录 |
| 2 | AI 回复时 | 写报告 + 更新记忆 |
| 3 | AI 执行完毕 | 记录变更清单 + 实时更新报告状态 |
| 4.5 | 任务结束前 | 执行合规性回溯检查（← 🆕 出口门禁） |
| 4 | 任务结束 | 最终状态 ✅ |
...
├── core/workflows/             ← 工作流定义（预检查、需求开发、Bug修复等）
├── core/templates/             ← 文档模板（lite/core/extended）
├── projects/              ← 各项目的规范和输出
│   └── <project>/
│       ├── profile/       ← 项目特定规范
...
A：参考 `QUICK-REFERENCE.md §任务类型映射`，或 `core/workflows/decision-tree.yaml` 中的关键词权重表。实在不确定就问用户。
...
| **任务记忆规范** | `core/workflows/common/task-memory.md` | 记忆文件格式/模板细节 |
| **报告规范** | `core/workflows/common/temp-reports.md` | 报告文件格式/存储规则 |
| **预检查工作流** | `core/workflows/00-pre-check/README.md` | 预检查详细步骤 |
