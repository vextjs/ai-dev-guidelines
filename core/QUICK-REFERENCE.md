...
| API 接口文档 | -（快速模式在聊天中直接输出精简版） | `core/api-doc-template.md` |
...
🔴 序号独立规则（v2.10.0 / task-memory v1.9 / temp-reports v1.7）:
...
🔴 消息驱动 5+1 阶段触发时机（task-memory v1.9）:
...
> ⚠️ 此清单新增文件时必须同步更新。独立版本号文件（如 task-memory.md v1.9、temp-reports.md v1.7）不在此清单中，见下方独立版本号检查规则。
