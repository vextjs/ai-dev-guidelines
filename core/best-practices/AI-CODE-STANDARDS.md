...
修复后必须执行的 3 步扫描:
  ...
  Step 2 - 数据联动扫描:
    ...
    数据联动清单（必查）:
      ...
      README.md 的工作流表 ←→ core/workflows/ 实际目录
...
### 0.3 方案-实施一致性（v2.0.3 新增）⭐

```yaml
原则: 执行阶段必须严格按实施方案执行，方案与实施必须始终保持一致

🔴 强制执行规则:
  1. 实施方案（03-implementation）经 CP3 确认后，执行阶段必须逐项对照执行
  2. 若在执行过程中发现方案存在问题（遗漏、错误、不可行）:
     a. 立即暂停当前执行
     b. 明确指出方案中的问题（具体到哪个文件/步骤/逻辑）
     c. 提出修订建议并更新实施方案文档
     d. 等待用户重新确认（走 CP3 确认流程）
     e. 确认后继续执行
  3. 执行完成后，方案文档与实际实施结果必须一致
     - 所有变更必须反写回方案文档
     - 方案中描述的文件列表/步骤/逻辑与实际执行零偏差

❌ 禁止行为:
  - 发现方案有问题但"默默修正"继续执行（不回溯方案）
  - 执行了方案中未列出的额外变更但不更新方案
  - 跳过方案中的某个步骤但不说明原因
  - 方案说改 A 文件，实际改了 B 文件但不更新方案

✅ 正确做法:
  - 方案 = 合同，执行 = 履约，偏差 = 变更合同后再履约
  - 任何实施偏差都通过"暂停→修订方案→确认→继续"闭环处理
```
...
// Step 1: 阅读库的实际代码和文档
// 查看: node_modules/redis/README.md 和 core/examples/
// 查看: 单元测试了解实际用法
