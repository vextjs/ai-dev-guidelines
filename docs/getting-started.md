...
ai-dev-guidelines 自带一组维护工具，在 `tools/` 目录下。你可以在终端中直接运行：

| 工具 | 做什么 | 怎么用 |
|------|--------|--------|
| **bump-version.js** | 版本号 + 约束条数自动同步到所有引用文件 | `node tools/bump-version.js --apply` |
| **add-constraint.js** | 新增约束的自动化向导 | `node tools/add-constraint.js --interactive` |
| **validate-links.js** | 检查文档链接是否有效 | `node tools/validate-links.js` |
| **validate-structure.js** | 检查目录结构是否规范 | `node tools/validate-structure.js` |
| **doc-health-check.js** | 文档综合健康检查 | `node tools/doc-health-check.js .` |
| **spec-health-check.js** | 规范一致性健康检查 | `node tools/spec-health-check.js` |
| **update-task-index.js** | 自动更新项目的任务索引 | `node tools/update-task-index.js <项目名>` |
...
**不需要。** 规范文件是给 AI 看的（`workflows/`、`CONSTRAINTS.md`、`QUICK-REFERENCE.md` 等）。你只需要读 `docs/` 目录下的用户指南，也就是本文件和上面链接的那些。
...
| 完整示例 | `ai-dev-guidelines/examples/` | 需求开发、Bug 修复示例 |