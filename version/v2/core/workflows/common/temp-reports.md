...
🔴 后续建议必须同时在对话中输出（🆕 v2.14.0）:
  触发: 报告中包含"后续建议"/"下一步"/"推荐改进"等行动项时
  规则:
    - 报告中: 写入完整的建议分析（含合理性/可实施性/收益验证）
    - 对话中: 必须同步输出建议摘要（标题 + 一句话说明 + 预期收益）
    - 禁止仅在报告中写入后续建议而对话中不提及
    - 禁止以"详见报告"作为唯一输出 — 对话中必须有实质内容
  已发生事故:
    - 会话07(vext OpenAPI): AI 在报告末尾写了 5 条后续建议，
      但对话中仅说"已完成，详见报告"，用户完全不知道有可操作建议，
      直到手动打开报告才发现，导致信息延迟
  详见: CONSTRAINTS.md 约束 #12 第 4 项
