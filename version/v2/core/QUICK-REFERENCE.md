...
│   └── 04-实施计划.md         #   🔴 实施计划（强制，任务编号+进度+中断恢复）
├── bugs/<中文描述>/
...
🔴 消息驱动 5+1 阶段触发时机:
  ...
  阶段 4:   任务结束时（最终状态更新 ✅ — 含报告头部状态 📝→✅ + 🗺️ 下一阶段建议（条件：项目有 实施计划）🆕 v2.12.0）
...
18. 🔴P0 **消息驱动记忆触发** - 记忆写入以消息事件为锚点（阶段 0~4），用户发消息时捕获输入（阶段 1），AI 回复时写报告（阶段 2），执行完毕记录变更（阶段 3）。🆕 编码任务（变更 ≥ 3 文件）触发**编码检查点**：有 实施计划 按任务编号、无则每 3 文件写入 📦 检查点（详见 triggers.md §编码检查点）
