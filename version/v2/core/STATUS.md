...
| 改进项 | 说明 |
|-------|------|
| 🎯 执行流程 | 统一 7 阶段流程（v2.13.0 FIX-016 移除快速/完整双模式） |
| 📝 精简模板 | `core/templates/lite/` 目录（小需求精简内容模板） |
| 🔄 工作流优化 | 需求开发统一 7 阶段 + CP1→CP5 |
| ✅ 预检查升级 | 5 项必做检查（含 Agent 标识 + 上次记忆） |
| 📋 项目规范 | 模块化 profile/ 结构（参考 chat 项目） |
| 🔍 快速索引 | 新增 core/best-practices/WHEN-TO-USE.md |
| 🔒 约束体系 | 22 条约束（含报告+记忆自动输出 + 消息驱动记忆触发 + 报告/记忆序号独立 + 文件过大必须拆分 + 编码后诊断检查 + Token 耗尽防护） |
...
| 00-task-identification/ | ✅ 完成 | v2.12.0: 4 层递进识别流程（含 Layer 3 意图推断） |
...
| 10-analysis/ | ✅ 完成 | 深度分析/审查/评估（含意图推断入口检测 Layer 3，v2.12.0） |
...
### v2.13.0 (当前版本) - 2026-03-06

**主题**: 统一 7 阶段流程 + Token 耗尽防护

- ✅ FIX-016：移除快速/完整双模式，所有需求开发统一为 7 阶段流程（25 个文件修改）
- ✅ 新增约束 #22「Token 耗尽防护」（保守预算制 + 三级防护阈值）
- ✅ issue#16 已解决（去掉快速模式）
- ✅ issue#17 已解决（Token 不足防护）

### v2.12.0 - 2026-03-05

**主题**: 预检查 Tool Call 硬性约束 + 约束#21 编码后诊断检查 + Layer 3 意图推断重构

- ✅ copilot-instructions.md 新增 §Tool Call 硬性约束（解决鸡生蛋问题，issue#10）
- ✅ 任务识别从 2 层+补丁重构为 4 层递进流程（Layer 3 意图推断三问，issue#2/#11 根因修复）
- ✅ FIX-010 关键词补丁升级为 Layer 3 正式识别层级
- ✅ 新增约束 #21"编码后诊断检查"，约束扩展至 21 条
- ✅ 五条绝对规则（3→5：新增禁止 glob 扫描 .ai-memory + 意图推断判定）
- ✅ issue#10/#11/#12 三条连环违规修复
- ✅ 14 个规范文件同步更新 + 交叉验证通过

### v2.11.0 - 2026-02-27 ~ 2026-03-02
...
- ✅ 新增快速/完整双模式（v2.13.0 已统一为单一 7 阶段流程）
...
| 版本 | 日期 | 主要变更 | 详情 |
|------|------|---------|------|
| v2.13.0 | 2026-03-06 | 统一7阶段流程(FIX-016移除快速/完整双模式)+约束#22(Token耗尽防护)+issue#16/#17修复 | 当前版本 |
| v2.12.0 | 2026-03-05 | Tool Call硬性约束+约束#21+Layer 3意图推断重构 | — |
| v2.11.0 | 2026-02-27 | 入口瘦身+SUMMARY精简+审计修复+版本号对齐 | — |
