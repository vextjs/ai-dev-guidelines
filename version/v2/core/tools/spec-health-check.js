...
function checkVersionConsistency() {
  ...
  const versionFiles = ['core/README.md', 'core/STATUS.md', 'core/QUICK-REFERENCE.md'];
  const versions = [];
  ...
  const dtPath = path.join(ROOT_DIR, 'core/workflows', 'decision-tree.yaml');
  if (fs.existsSync(dtPath)) {
    const content = fs.readFileSync(dtPath, 'utf-8');
    const match = content.match(/version:\s*["']?([\d.]+)["']?/);
    if (match) {
      versions.push({ file: 'core/workflows/decision-tree.yaml', version: match[1] });
    }
  }
