...
分析流程中，AI **绝对禁止**执行以下操作：

| 禁止操作 | 说明 |
|---------|------|
| `edit_file()` 修改项目源码 | 只能读取，不能修改 `src/` `lib/` `app/` 等 |
| `edit_file()` 修改项目配置 | 只能读取，不能修改 `package.json` `tsconfig.json` 等 |
| `delete_path()` 删除项目文件 | 不能删除任何项目文件 |
| `move_path()` 移动项目文件 | 不能移动或重命名任何项目文件 |
| `terminal` 执行修改命令 | 不能执行 `npm install` `git commit` 等修改性命令 |
...
| `diagnostics()` | 获取诊断信息 |
| `terminal` 只读命令 | `git log` · `git diff` · `npm list` · `cat` 等 |
| `edit_file()` 写入报告/记忆 | 写入 `reports/` 和 `.ai-memory/` 是正常行为 |
