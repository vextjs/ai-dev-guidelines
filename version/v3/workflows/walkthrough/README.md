...
| # | 检查项 | 期望行为 |
|:-:|--------|---------|
| G1 | 时间戳获取 | AI 首先获取当前时间（系统上下文注入优先，降级为原生工具，再降级为终端命令） |
| G2 | 记忆目录扫描 | 逐层列出目录内容进入 `.ai-memory/`（🔴 禁止 glob） |
...
| 6 | E09 | AI 绕过管线直接行动 | 🔴 不跳过预检查 + 必须声明路由 + 修改前确认 |
| 7 | E01 | fix vs build 边界判定 | ≥5 文件时建议切换 · CP 差异（D26③） |
