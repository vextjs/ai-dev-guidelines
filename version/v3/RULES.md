...
| 12 | 任务完成验证 | 声称完成前 7 项验证：需求覆盖+报告存在+记忆完整+CP完整+阶段4.5通过+约束遵守+工作流验证 |
...
1. ✅ 需求覆盖（用户所有需求点已处理）
2. ✅ 报告存在（报告文件已写入且可读）
3. ✅ 记忆完整（任务摘要+对话记录+关联报告）
4. ✅ CP 完整（必经 CP 已按序通过，build: CP1~CP3 / fix: CP1~CP2）
5. ✅ 阶段 4.5 通过（合规性回溯检查已通过）
6. ✅ 约束遵守（关键约束已遵守+关联文件已同步）
7. ✅ 工作流验证（工作流专属验证清单已通过）
